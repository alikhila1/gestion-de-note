...
public class EtudiantLocalRestController {
	...
	void ajouterEtudiantLocal(@RequestBody Etudiant_local e, @PathVariable("idGroupe") int idGroupe)
	...
	public Optional<Etudiant_local> getEtudianLocalLogged(@RequestParam String username,
			@RequestParam String password) {
		return (iEtudiantLocalService.getEtudiantLocalLogged(username, password));
	}

	@GetMapping("/getEtudiantLocalLog")
	public List<Map<String, Object>> listEtudiantLocalsLogged(@RequestParam String username,
			@RequestParam String password) {
		String sql = "SELECT user_id,user_mail,user_nom,user_pre,user_name FROM T_ETUD_LOCAL where user_name='"
	...
	public ResponseEntity<?> uploadEtudiantsData(@RequestPart(value = "file", required = true) MultipartFile file,
			@RequestPart(value = "nomMatiere", required = true) String nomMatiere,
			@RequestPart(value = "grpId", required = true) String grpId) {
		this.iEtudiantLocalService.saveEtudiantsToDatabase(file, nomMatiere, grpId);
		return ResponseEntity.ok(Map.of("Message", " Etudiants data uploaded avec succes"));
	}
	...
	public List<Map<String, Object>> listEtudiantLocalsWNotes(@PathVariable("idMat") int id) {
		String sql = "SELECT e.*,en.*,n.* FROM t_etud_local e join t_etud_local_notes en ON e.user_id=en.etudiants_user_id join t_note n on en.notes_note_id=n.note_id where mat_matiere_id="
				+ id;
		return jdbcTemplate.queryForList(sql);
	}
	...
	public List<Map<String, Object>> EtudiantLocalsWNotes(@PathVariable("idEtd") int id) {
		String sql = "SELECT DISTINCT m.matiere_nom,m.matiere_coef,n.* FROM t_etud_local e join t_etud_local_notes en ON e.user_id=en.etudiants_user_id join t_note n on en.notes_note_id=n.note_id join t_matiere m on n.mat_matiere_id=m.matiere_id where e.user_id="
				+ id;
		return jdbcTemplate.queryForList(sql);
	}
	...
	@CrossOrigin(origins = "http://localhost:4200")
	public ResponseEntity<?> uploadEtudiantsDataAbsence(
