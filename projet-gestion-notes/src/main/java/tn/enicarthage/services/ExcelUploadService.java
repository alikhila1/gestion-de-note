...
public class ExcelUploadService {
	...
	public static List<Etudiant_local> getEtudiantsDataFromExcel(InputStream inputStream, String nomMatiere,
		...
		try {
			...
			XSSFSheet sheet = workbook.getSheet("notes");
			int rowIndex = 0;
			for (Row row : sheet) {
				if (rowIndex == 0) {
					rowIndex++;
		...
		for (Etudiant_local etd : etudiants) {
			try {
				MimeMessage message = mailSender.createMimeMessage();
				MimeMessageHelper helper = new MimeMessageHelper(message, true);
				String mailSubject = "Bienvenue chez Ecole nationale d'ingénieur de carthage";
				String mailContent = "<p><b>Votre nom d'utilisateur est :</b>" + etd.getUsername() + "</p>";
				mailContent += "<p><b>Votre mot de passe est :</b>" + etd.getPassword() + "</p>";
